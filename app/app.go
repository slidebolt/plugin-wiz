...
func init() { domain.Register("wiz_light", WizLightState{}) }
...
func (a *App) discoverAndRegister() error {
	...
	for _, dev := range devices {
		...
		state := WizLightState{Power: false, Brightness: 100}
		if dev.GetPilot != nil {
			if power, ok := dev.GetPilot["state"].(bool); ok {
				state.Power = power
			}
			if dimming, ok := dev.GetPilot["dimming"].(float64); ok {
				state.Brightness = int(dimming)
			}
			if temp, ok := dev.GetPilot["temp"].(float64); ok {
				state.Temperature = int(temp)
			}
			if r, ok := dev.GetPilot["r"].(float64); ok {
				if g, ok := dev.GetPilot["g"].(float64); ok {
					if b, ok := dev.GetPilot["b"].(float64); ok {
		...
		entity := domain.Entity{
			ID: entityID, Plugin: PluginID, DeviceID: deviceID, Type: "wiz_light",
			...
			State:    state,
			Meta:     map[string]json.RawMessage{"ip": json.RawMessage(fmt.Sprintf(`"%s"`, dev.IP)), "mac": json.RawMessage(fmt.Sprintf(`"%s"`, dev.Mac))},
		}
...
func (a *App) handleCommand(addr messenger.Address, cmd any) {
	...
	case domain.LightTurnOn:
		if err := wiz.SetPilot(ip, map[string]any{"state": true}); err == nil {
			a.updateLightState(entity, func(s *WizLightState) { s.Power = true })
		}
	case domain.LightTurnOff:
		if err := wiz.SetPilot(ip, map[string]any{"state": false}); err == nil {
			a.updateLightState(entity, func(s *WizLightState) { s.Power = false })
		}
	case domain.LightSetBrightness:
		if err := wiz.SetPilot(ip, map[string]any{"state": true, "dimming": c.Brightness}); err == nil {
			a.updateLightState(entity, func(s *WizLightState) { s.Power = true; s.Brightness = c.Brightness })
		}
	case domain.LightSetColorTemp:
		kelvin := 1000000 / c.Mireds
		if err := wiz.SetPilot(ip, map[string]any{"state": true, "temp": kelvin}); err == nil {
			a.updateLightState(entity, func(s *WizLightState) { s.Power = true; s.Temperature = kelvin })
		}
	case domain.LightSetRGB:
		if err := wiz.SetPilot(ip, map[string]any{"state": true, "r": c.R, "g": c.G, "b": c.B}); err == nil {
			a.updateLightState(entity, func(s *WizLightState) { s.Power = true; s.RGB = []int{c.R, c.G, c.B} })
		}
	default:
...
func (a *App) updateLightState(entity domain.Entity, mutate func(*WizLightState)) {
	state, ok := entity.State.(WizLightState)
	if !ok {
		if stateMap, ok := entity.State.(map[string]interface{}); ok {
			if power, ok := stateMap["power"].(bool); ok {
				state.Power = power
			}
			if brightness, ok := stateMap["brightness"].(float64); ok {
				state.Brightness = int(brightness)
			}
			if temperature, ok := stateMap["temperature"].(float64); ok {
