...
// ==========================================================================
// Light entity (domain.Light)
// ==========================================================================

func TestCustom_WizLight_SaveGetHydrate(t *testing.T) {
	_, store, _ := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-aabbccdd", "wiz-aabbccdd", "light", "Living Room Bulb",
		domain.Light{Power: true, Brightness: 75, RGB: []int{255, 100, 0}})

	got := getEntity(t, store, "plugin-wiz", "wiz-aabbccdd", "wiz-aabbccdd")
	state, ok := got.State.(domain.Light)
	if !ok {
		t.Fatalf("state type: got %T, want domain.Light", got.State)
	}
...
func TestCustom_WizLight_QueryByType(t *testing.T) {
	_, store, _ := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-001", "wiz-001", "light", "Bulb 1", domain.Light{Power: true})
	saveEntity(t, store, "plugin-wiz", "wiz-002", "wiz-002", "light", "Bulb 2", domain.Light{Power: false})
	saveEntity(t, store, "test", "dev1", "light001", "light", "Regular Light", domain.Light{Power: true})

	entries := queryByType(t, store, "light")
	if len(entries) != 3 {
		t.Fatalf("lights: got %d, want 3", len(entries))
	}
}
...
func TestLight_TurnOn(t *testing.T) {
	_, store, cmds := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-001", "wiz-001", "light", "Test Bulb", domain.Light{Power: false})

	entity := domain.Entity{ID: "wiz-001", Plugin: "plugin-wiz", DeviceID: "wiz-001", Type: "light"}
...
func TestLight_TurnOff(t *testing.T) {
	_, store, cmds := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-001", "wiz-001", "light", "Test Bulb", domain.Light{Power: true})
...
func TestLight_SetBrightness(t *testing.T) {
	_, store, cmds := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-001", "wiz-001", "light", "Test Bulb", domain.Light{Brightness: 50})
...
func TestLight_SetColorTemp(t *testing.T) {
	_, store, cmds := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-001", "wiz-001", "light", "Test Bulb", domain.Light{Temperature: 2700})
...
func TestLight_SetRGB(t *testing.T) {
	_, store, cmds := env(t)
	saveEntity(t, store, "plugin-wiz", "wiz-001", "wiz-001", "light", "Test Bulb", domain.Light{})
